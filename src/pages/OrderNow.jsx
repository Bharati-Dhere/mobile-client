...
export default function OrderNow() {
    ...
    const fetchUserWishlist = async () => {
      if (showWishlistModal) {
        try {
          ...
          const res = await axios.get(`https://mobile-server-1.onrender.com/api/users/by-external-id/${user.id}`);
  ...
  const handlePlaceOrder = async () => {
    if (payment.method === 'UPI') {
      ...
      try {
  const orderRes = await fetch('https://mobile-server-1.onrender.com/create-order', {
          method: 'POST',
        ...
        const options = {
          ...
          handler: async function (response) {
            const verifyRes = await fetch('https://mobile-server-1.onrender.com/verify-payment', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
